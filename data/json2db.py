...
def json2table():
    ...
    for json_path in json_paths:
        ...
        with open(json_path, "r", encoding="utf-8") as f:
            ...
            # jsonデータをINSERTする
            if info_name == "base":
                all_sql = insert_sql + "(appid, name, publisher, short_description, about_the_game) VALUES (?, ?, ?, ?, ?)"
                for obj in data:
                    rows.append([obj["appid"], obj["name"], json.dumps(obj["publisher"], ensure_ascii=False), obj["short_description"], obj["about_the_game"]])
            elif info_name == "image":
                all_sql = insert_sql + "(appid, capsule, capsule_v5, header, screenshots) VALUES (?, ?, ?, ?, ?)"
                for obj in data:
                    rows.append([obj["appid"], obj["capsule"], obj["capsule_v5"], obj["header"], json.dumps(obj["screenshots"], ensure_ascii=False)])
